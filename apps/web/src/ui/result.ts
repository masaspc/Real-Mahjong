 ...
 * 局の結果を出す。
 *
 * 必要な値はすべて `agari` イベントが運んでいる（役と翻、符、点数、裏ドラ、
 * 点棒の増減）。出していなかっただけである。
 *
 * **進行はサーバが止める。**局が終わってから次の局を配るまでに間を置く
 * （`server::session::INTERLUDE_MS`）。この区間ではどの席にも行動要求が
 * 出ていないので、誰の持ち時間も減らない。板はその間より少しだけ長く残す。
 */
 ...
 * 結果を出しておく長さ。
 *
 * **サーバが局と局のあいだに置く間（6秒）より少し長くする。**間が明けて
 * 次の局が配られ始めても、読み終わるまでは残っていてほしい。逆に長すぎると
 * 次の局の邪魔になる。**サーバの定数と結び付けてはいない**（`protocol` の
 * 契約ではないので、共有すると凍結の意味が濁る）。「間より少し長い」という
 * 関係だけを守る。
 */
export const RESULT_SHOWN_MS = 9_000;
