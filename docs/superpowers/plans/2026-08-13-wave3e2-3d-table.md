...
- **`layout.ts` はこのウェーブが直した。**卓の寸法そのものが破綻していた。山は一辺17枚ぶんあるのに中心から 6.2 に置かれており、**四辺が卓の角で交差する。**河も4段目が中心へ寄りすぎて、対面とも隣席とも重なる。卓を 26×26 とし、山を 11.5、手牌を 10、河の1段目を 7.775 へ置き直した。すべての牌を総当たりで見て重なりが無いことをテストで固定してある。**凍結は並行作業の衝突を防ぐ取り決めであり、実際の欠陥を直すことを禁じるものではない。**
...
function overlappingPairs(all: ReturnType<typeof placementsFor>): string[] {
  ...
  for (let i = 0; i < solid.length; i += 1) {
    for (let j = i + 1; j < solid.length; j += 1) {
      ...
      const b = solid[j];
      if (!a || !b) continue;
      // 同じ席の同じ種類は隣り合って並ぶので触れ合ってよい。
      if (a.seat === b.seat && a.kind === b.kind) continue;
    ...
    laidMelds.forEach((pieces, meldIndex) => {
      ...
      pieces.forEach((piece, tileIndex) => {
        ...
        out.push({
          ...
          position: rowPosition(relative, column, total, piece.stack * TILE.height),
          rotationX: 0,
...
- カメラは俯瞰のまま。卓が広がったぶん引く（`position.set(0, 18, 17)` あたり）。`resize` と `render` と `dispose` はそのまま使う
