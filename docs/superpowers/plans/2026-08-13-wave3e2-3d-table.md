 ...
 * 牌の姿勢。
 *
 * **Three.js の `rotation.order` を `"YXZ"` にする前提。**その順序では
 * 合成が `Ry · Rx · Rz` になり、「自分の絵柄の面で回してから、寝かせて、
 * 席の向きへ回す」という意図どおりになる。既定の `"XYZ"` は
 * `Rz · Ry · Rx` なので、席が回ると牌の面が上や下を向いてしまう。
 */
function orientation(p: { rotationX: number; rotationY: number; rotationZ: number }): number[][] {
  return mul(rotY(p.rotationY), mul(rotX(p.rotationX), rotZ(p.rotationZ)));
}

/** 牌の面が向いている先。 */
function faceNormal(p: { rotationX: number; rotationY: number; rotationZ: number }) {
 ...
 * 隣席の河どうしの重なりと、山の四辺が角で交差することを
 * 見落としていた。すべての牌を総当たりで見る。
 */
...
describe("盤面から牌の置き場所を出す", () => {
  ...
  it("鳴いた牌は横に倒し、取得元で置く場所が変わる", () => {
    ...
    expect(fromLeft.map((p) => p.rotationZ !== 0)).toEqual([true, false, false]);

    // 席1が席2（下家）から鳴いたら右端が倒れる。
    const fromRight = ponFrom(2, 1);
    expect(fromRight.map((p) => p.rotationZ !== 0)).toEqual([false, false, true]);

    // 席1が席3（対面）から鳴いたら真ん中が倒れる。
    const fromAcross = ponFrom(3, 1);
    expect(fromAcross.map((p) => p.rotationZ !== 0)).toEqual([false, true, false]);
  });
  ...
  it("大明槓は4枚並び、鳴いた牌が倒れる", () => {
    ...
    const melds = placementsFor(state).filter((p) => p.kind === "meld");
    expect(melds).toHaveLength(4);
    expect(melds.filter((p) => p.rotationZ !== 0)).toHaveLength(1);
    // 上家から鳴いたので左端。
    expect(melds[0]?.rotationZ).not.toBe(0);
    expect(melds.every((p) => p.faceUp)).toBe(true);
  });
  ...
  it("手牌と副露は立て、河と山は寝かせる", () => {
    ...
    const all = placementsFor(state);
    const standing = all.filter((p) => p.kind === "hand" || p.kind === "meld");
    const lying = all.filter((p) => p.kind === "river" || p.kind === "wall");
  ...
  it("チーで倒れるのは鳴いた牌そのもの", () => {
    ...
    const melds = placementsFor(state).filter((p) => p.kind === "meld");
    const sideways = melds.filter((p) => p.rotationZ !== 0);
  ...
  it("加槓で倒れるのは足した牌ではなく鳴いた牌の位置", () => {
    ...
    expect(called?.rotationZ).not.toBe(0);
    expect(fourth?.rotationZ).not.toBe(0);
    ...
    // 真ん中の2枚は立ったまま。
    expect(melds[1]?.rotationZ).toBe(0);
    expect(melds[2]?.rotationZ).toBe(0);
  });
...
export type Placement = {
   ...
   * 絵柄の面のなかで回す角度。
   *
   * **副露の鳴いた牌はこれで倒す。**`rotationY` で回すと牌の面が横を
   * 向いてしまい、何の牌か見えなくなる。河の宣言牌は寝ているので
   * `rotationY` でよい（面が上を向いたまま回る）。
   */
  rotationZ: number;
  faceUp: boolean;
  /**
 ...
 */
function rowPosition(relative: number, slot: number, total: number, lift = 0): Vec3 {
  const spacing = Math.min(TILE.width, (ROW_HALF_WIDTH * 2) / Math.max(total, 1));
  return rotateY(
    { x: (slot - (total - 1) / 2) * spacing, y: TILE.height / 2 + lift, z: ROW_Z },
...
export function placementsFor(state: GameState, viewer: Seat = state.you): Placement[] {
    ...
    laidMelds.forEach((pieces, meldIndex) => {
      ...
      pieces.forEach((piece, tileIndex) => {
        ...
        out.push({
          ...
          position: rowPosition(relative, column, total, piece.stack * TILE.width),
          rotationX: 0,
          rotationY: facing,
          rotationZ: piece.sideways ? SIDEWAYS : 0,
          faceUp: piece.faceUp,
          pickable: false,
        });
        if (piece.stack === 0) {
  ...
  for (let index = 0; index < state.wallRemaining; index += 1) {
    ...
    const side = Math.floor(slot / SLOTS_PER_SIDE);
    out.push({
      key: `wall-${slot}`,
      kind: "wall",
      seat: side as Seat,
      encoded: 0,
      position: wallPosition(side, slot % SLOTS_PER_SIDE),
      rotationX: LYING,
...
- **回す前に `mesh.rotation.order = "YXZ"` を必ず設定する。**そのうえで `rotation.set(rotationX, rotationY, rotationZ)` を呼ぶ。
  この順序では合成が `Ry · Rx · Rz` になり、「自分の絵柄の面で回してから、寝かせて、席の向きへ回す」という意図どおりになる。
  **Three.js の既定は `"XYZ"` で、合成は `Rz · Ry · Rx`。**そのままだと席1の鳴いた牌の面が真上を、席3のものが真下を向き、絵柄が見えなくなる
