...
function footprint(p: {
  ...
  const lying = Math.abs(p.rotationX) > 0.01;
  // 立てた牌は x=1・z=0.6、寝かせた牌は x=1・z=1.35。
  let w = 1;
  let d = lying ? 1.35 : 0.6;
  // 絵柄の面のなかで倒すと、幅と高さが入れ替わる（幅 1.35 になる）。
  if (Math.abs(Math.sin(p.rotationZ)) > 0.5) {
    w = 1.35;
  }
  // 横に倒すと x と z が入れ替わる。
  if (Math.abs(Math.sin(p.rotationY)) > 0.5) {
    [w, d] = [d, w];
  }
  return {
    x0: p.position.x - w / 2,
