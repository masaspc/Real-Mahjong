...
- Create: `apps/web/preview.html`
- Create: `apps/web/src/preview.ts`
- Modify: `apps/web/vite.config.ts`
- Modify: `apps/web/src/scene/atlas.ts`
- Modify: `apps/web/src/scene/atlas.test.ts`
- Modify: `apps/web/src/scene/face-atlas.test.ts`
...
**対局しながら目で確かめてはいけない。**加槓や暗槓は出るまで打つしかなく、
出ない半荘もある。**運に完了条件を預けると、ここで必ず止まる。**

`apps/web/preview.html` と `apps/web/src/preview.ts` を作り、副露5種・
リーチ宣言牌・21枚の河・山を最初から並べた盤面を、本物の `TableScene` で
描く。`?viewer=0..3` で見る席を選べるようにする（押さずに席ごとの絵を
撮るため）。`vite.config.ts` の `build.rollupOptions.input` に
`preview.html` を足す。**足さないとビルドに入らない。**

判定は通さない。実際の局で作れる手牌である必要もない。**確かめたいのは
置き場所と牌の面だけ。**

- [ ] **Step 4: 牌の面が本当に貼れているか確かめる**

ヘッドレスの Chrome で撮る。**WebGL は swiftshader で動く。**

Run: `pnpm --dir apps/web build && cargo run -p server --bin serve &` してから
```bash
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  ...
  --screenshot=preview.png "http://127.0.0.1:8080/preview.html?viewer=0"
```
Expected: `preview.png` が書き出され、河に `一萬 九筒 三索 東 七萬 二筒 / 白 八索 四萬` が
`preview.ts` に書いた並びのとおりに出る。**文字ラベル（`1m` `9p`）が出たら失敗。**
仮のアトラスのままで、SVG の牌面が焼けていない。

この2つは実際に踏んだ罠である。**どちらも 168 件の試験を素通りした。**

- **アトラスの行が上下反転する。**`uvOffsetOf` の `v` は canvas の行と逆向き
  （テクスチャの `v=0` は下端）。`row * dv` と書くと東が3pに、牌体が7mに化ける。
  **真ん中の行だけ偶然正しくなる。**
- **`tile-face.ts` の SVG に `xmlns` が要る。**`innerHTML` なら省けるが、
  `data:` URL の画像は独立した文書として解析される。`paintAtlas` は39枚を
  `Promise.all` で待つので、**1枚欠けると全部が仮のラベルのまま出る。**

Run: `pnpm --dir apps/web test`
Expected: 171 passed（行の向きと `xmlns` を見る試験を含む）
...
git add apps/web/src/ui/board.ts apps/web/src/ui/board.css apps/web/src/main.ts \
  apps/web/preview.html apps/web/src/preview.ts apps/web/vite.config.ts \
  apps/web/src/scene/atlas.ts apps/web/src/scene/atlas.test.ts \
  apps/web/src/scene/face-atlas.test.ts apps/web/src/scene/tile-geometry.test.ts \
  apps/web/src/ui/tile-face.ts
...
**ここから先はループに乗せない。**「麻雀牌に見えるか」「卓が綺麗か」は
主観であり、機械では合否が決まらない。`【要確認】` を付けて人間に渡す。

渡すもの: `?viewer=0` と `?viewer=2` の画。**遊ばせない。**
