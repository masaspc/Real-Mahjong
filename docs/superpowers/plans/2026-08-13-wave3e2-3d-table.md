...
function within(p: Parameters<typeof footprint>[0]): boolean {
  const f = footprint(p);
  return f.x0 > -10 && f.x1 < 10 && f.z0 > -10 && f.z1 < 10;
}

/** 別の席の牌どうしで重なっている組を返す。 */
function overlappingPairs(all: ReturnType<typeof placementsFor>): string[] {
  const pairs: string[] = [];
  const solid = all.filter((p) => p.kind === "hand" || p.kind === "drawn" || p.kind === "meld");
 ...
 * 手牌と副露を並べる列の、片側の幅の上限。
 *
 * **隣の席の列と卓の角でぶつかってはいけない。**各席の列は z=7.5
 * にあり、隣席では回転して x=7.5 になる。立てた牌は厚み 0.6 を
 * 占めるので隣席の列は 7.2..7.8。こちらの牌の端が 6.7 を超えると
 * 角で重なる。
 */
const ROW_HALF_WIDTH = 6.7;
