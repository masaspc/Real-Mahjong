...
- **`layout.ts` はこのウェーブが直した。**河の1段目の奥行きが 4.2 だと、4段目が z=0.15 に来て**対面の河と重なる**。寝かせた牌は z 方向に 1.35 を占めるためである。4.8 へ直し、対面と重ならないこと・山へ食い込まないことをテストで固定した。**凍結は並行作業の衝突を防ぐ取り決めであり、実際の欠陥を直すことを禁じるものではない。**
...
describe("盤面から牌の置き場所を出す", () => {
  ...
  it("鳴いた牌は横に倒し、取得元で置く場所が変わる", () => {
    ...
    expect(fromLeft.map((p) => p.rotationY !== fromLeft[1]?.rotationY)).toEqual([true, false, false]);

    // 席1が席2（下家）から鳴いたら右端が倒れる。
    const fromRight = ponFrom(2, 1);
    expect(fromRight.map((p) => p.rotationY !== fromRight[0]?.rotationY)).toEqual([false, false, true]);

    // 席1が席3（対面）から鳴いたら真ん中が倒れる。
    const fromAcross = ponFrom(3, 1);
    expect(fromAcross.map((p) => p.rotationY !== fromAcross[0]?.rotationY)).toEqual([false, true, false]);
  });
  ...
  it("大明槓は4枚並び、鳴いた牌が倒れる", () => {
    ...
    const melds = placementsFor(state).filter((p) => p.kind === "meld");
    expect(melds).toHaveLength(4);
    expect(melds.filter((p) => p.rotationY !== 0)).toHaveLength(1);
    // 上家から鳴いたので左端。
  ...
  it("副露が増えても卓からはみ出さない", () => {
    ...
    // 卓は 20×20。半分は 10。
    expect(melds.every((p) => Math.abs(p.position.x) < 10 && Math.abs(p.position.z) < 10)).toBe(true);
  });
  ...
  it("チーで倒れるのは鳴いた牌そのもの", () => {
    ...
    const melds = placementsFor(state).filter((p) => p.kind === "meld");
    // 席0を席0から見るので、立っている牌の向きは 0。
    const sideways = melds.filter((p) => p.rotationY !== 0);
  ...
  it("加槓で倒れるのは足した牌ではなく鳴いた牌の位置", () => {
    ...
    const melds = placementsFor(state).filter((p) => p.kind === "meld");
    expect(melds).toHaveLength(4);
    // 上家から鳴いたので左端が倒れ、足した4枚目も倒れる。
    const upright = melds[1]?.rotationY ?? 0;
    expect(melds.map((p) => p.rotationY !== upright)).toEqual([true, false, false, true]);
  });
 ...
 * 副露を置く右端。
 *
 * **手牌の列を伸ばして置いてはいけない。**副露が増えるたび外へ伸び、
 * 2つ目で卓（20×20）の外へ出る。右端から内へ向けて詰める。
 */
const MELD_ANCHOR_X = 8.5;
...
/** 副露の1枚を置く場所。右端から左へ詰める。 */
function meldPosition(relative: number, column: number): Vec3 {
  return rotateY(
    { x: MELD_ANCHOR_X - column * TILE.width, y: TILE.height / 2, z: MELD_Z },
    seatRotation(relative),
  );
}

type MeldPiece = { tile: Tile; faceUp: boolean; sideways: boolean };
...
export function layMeld(meld: MeldView, owner: Seat): MeldPiece[] {
  ...
  if (meld.kind === "kakan") {
    // 元のポン3枚に4枚目を足したもの。鳴いた牌と足した牌の2枚を倒す。
    const slot = slotOf(3);
    return tiles.map((tile, index) => ({
      tile,
      faceUp: true,
      sideways: index === slot || index === tiles.length - 1,
    }));
  }

  // チー・ポン・大明槓。エンジンは鳴いた牌を末尾へ入れる。
  const called = tiles.pop();
  const pieces: MeldPiece[] = tiles.map((tile) => ({ tile, faceUp: true, sideways: false }));
  if (called !== undefined) {
    pieces.splice(slotOf(pieces.length + 1), 0, { tile: called, faceUp: true, sideways: true });
  }
  return pieces;
}
  ...
  for (let absolute = 0; absolute < 4; absolute += 1) {
    ...
    if (mine && state.drawn !== null) {
      out.push({
        ...
        seat: absolute,
        encoded: state.drawn,
        position: handPosition(relative, size + 1, size + 2),
        rotationX: 0,
        rotationY: facing,
        faceUp: true,
        pickable: true,
    ...
    seat.melds.forEach((meld, meldIndex) => {
      layMeld(meld, absolute).forEach((piece, tileIndex) => {
        out.push({
          key: `meld-${absolute}-${meldIndex}-${tileIndex}`,
          kind: "meld",
          seat: absolute,
          encoded: piece.faceUp ? piece.tile : 0,
          position: meldPosition(relative, meldColumn),
          rotationX: 0,
...
- 加槓は4枚を横一列に並べ、鳴いた牌と足した牌の2枚を倒す。本来は元のポンの上へ重ねる。
- 加槓は4枚を横一列に並べる。本来は元のポンの上へ4枚目を重ねる。
