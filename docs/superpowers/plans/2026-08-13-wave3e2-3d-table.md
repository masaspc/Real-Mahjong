 ...
 */
function layMeld(meld: { kind: string; tiles: Tile[]; from: Seat }, owner: Seat): MeldPiece[] {
  ...
  // エンジンは鳴いた牌を末尾へ入れる。表示は取得元の位置へ移す。
  const called = tiles.pop();
  const source = relativeSeat(meld.from, owner);
  const slot = source === 3 ? 0 : source === 2 ? 1 : tiles.length;
  const pieces: MeldPiece[] = tiles.map((tile) => ({ tile, faceUp: true, sideways: false }));
  if (called !== undefined) {
    pieces.splice(slot, 0, { tile: called, faceUp: true, sideways: true });
  }
  return pieces;
}
...
export function placementsFor(state: GameState, viewer: Seat = state.you): Placement[] {
  ...
  for (let absolute = 0; absolute < 4; absolute += 1) {
    ...
    seat.melds.forEach((meld, meldIndex) => {
      const laid = layMeld(meld, absolute);
      laid.forEach((piece, tileIndex) => {
        ...
        out.push({
          ...
          position: {
            x: spot.x + MELD_OFFSET * Math.cos(facing),
            y: spot.y,
            z: spot.z - MELD_OFFSET * Math.sin(facing),
          },
  ...
  // たびに4辺が順番に欠けていき、山に見えない。実際の山は一方の端から
  // 連続して減る。
...
- `pickHandTile(x, y)`: `Raycaster` で当たった牌のうち、**`pickable` が真のものだけを返す。**`kind` で絞ってはいけない。他家の伏せ牌も `kind` は `"hand"` であり、中身に 0（一萬）が入っているので、**押すと一萬を切ってしまう**
