...
| 検査 | 内容 |
|---|---|
| 牌の枚数 | 手牌は13枚（副露があれば `13 − 副露数×3`）。和了牌を加えて14枚 |
| 記法の妥当性 | 字牌は 1z..7z のみ。数牌は 0..9（0は赤ドラ） |
| 和了形かどうか | 和了すると書いたケースが標準形・七対子・国士のいずれかで実際に和了する |
| 文書内の整合 | 冒頭の方針と後半のコード例が矛盾していないか |
| 文字の混入 | 日本語・ASCII 以外の文字が紛れていないか |
